...
class ICMPv4Socket:
    ...
    def _create_ICMP_packet(self, id, sequence, payload):
        checksum = 0
        header = pack(
            "!2B3H", self._ICMP_ECHO_REQUEST, 0, checksum, id, sequence
        )
    ...
    def _parse_reply(self, packet, source, current_time):
        ...
        type, code = unpack(
            "!2B",
            packet[self._ICMP_HEADER_OFFSET : self._ICMP_CHECKSUM_OFFSET],
        )
        if type != self._ICMP_ECHO_REPLY:
            packet = packet[self._ICMP_PAYLOAD_OFFSET :]
        id, sequence = unpack(
            "!2H", packet[self._ICMP_ID_OFFSET : self._ICMP_PAYLOAD_OFFSET]
        )
    ...
    def send(self, request):
        sock_destination = socket.getaddrinfo(
            port=0, host=request.destination, family=self._sock.family
        )[0][4]
        self._address = request.destination
        packet = self._create_ICMP_packet(
            id=request.id, sequence=request.sequence, payload=request.payload
        )
    ...
    def receive(self, request=None, timeout=2):
        ...
        while True:
            ...
            reply = self._parse_reply(
                packet=packet, source=source, current_time=current_time
            )
            if (
                reply
                and not request
                or reply
                and request.id == reply.id
                and request.sequence == reply.sequence
            ):
                return reply
