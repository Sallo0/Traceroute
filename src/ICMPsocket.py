...
class ICMPv4Socket:
    ...
    def _create_ICMP_packet(self, id, sequence, payload):
        checksum = 0
        header = pack("!2B3H", self._ICMP_ECHO_REQUEST, 0, checksum, id, sequence)
    ...
    def send(self, request):
        sock_destination = socket.getaddrinfo(port=0, host=request.destination, family=self._sock.family)[0][4]
        self._address = request.destination
        packet = self._create_ICMP_packet(id=request.id, sequence=request.sequence, payload=request.payload)
    ...
    def receive(self, request=None, timeout=2):
        ...
        while True:
            ...
            packet = response[0]
            source = response[1][0]
            reply = self._parse_reply(packet=packet, source=source, current_time=current_time)
